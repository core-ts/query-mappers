...
export type Build = (template: Template, obj: any, param: (i: number) => string) => Statement;
...
export function useQuery(id?: string, mapper?: Map<string, Template>, build?: Build, attrs?: Attributes, useContain?: boolean, q?: string, sort?: string): Query | undefined {
  if (id && mapper && build) {
    const template = mapper.get(id);
    if (template) {
      const query = useQueryBuilder(template, build, attrs, useContain, q, sort);
...
export function useQueryBuilder<S>(template: Template, build: Build, attributes?: Attributes, useContain?: boolean, q?: string, sort?: string): QueryBuilder<S> {
  return new QueryBuilder<S>(template, build, useContain, undefined, attributes, q, sort);
}
export class QueryBuilder<S> {
  constructor(public template: Template, public build: Build, public useContain?: boolean, bf?: BuildFilter, public attributes?: Attributes, q?: string, sort?: string) {
