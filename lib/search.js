...
function useQuery(id, mapper, attrs, useContain, skipArray, q, sort) {
  if (id && mapper) {
    var template = mapper.get(id);
    if (template) {
      var query = useQueryBuilder(template, attrs, useContain, skipArray, q, sort);
...
function useQueryBuilder(template, attributes, useContain, skipArray, q, sort) {
  return new QueryBuilder(template, useContain, undefined, attributes, q, sort, skipArray);
}
exports.useQueryBuilder = useQueryBuilder;
var QueryBuilder = (function () {
  function QueryBuilder(template, useContain, bf, attributes, q, sort, skipArray) {
  ...
  QueryBuilder.prototype.buildQuery = function (filter, bparam, sn, buildSort, attrs) {
    ...
    return core_1.build(f2, this.template, param1, this.skipArray);
  };
  return QueryBuilder;
}());
