...
function useQuery(id, mapper, build, attrs, useContain, skipArray, q, sort) {
  if (id && mapper && build) {
    var template = mapper.get(id);
    if (template) {
      var query = useQueryBuilder(template, build, attrs, useContain, skipArray, q, sort);
...
function useQueryBuilder(template, build, attributes, useContain, skipArray, q, sort) {
  return new QueryBuilder(template, build, useContain, undefined, attributes, q, sort, skipArray);
}
exports.useQueryBuilder = useQueryBuilder;
var QueryBuilder = (function () {
  function QueryBuilder(template, build, useContain, bf, attributes, q, sort, skipArray) {
