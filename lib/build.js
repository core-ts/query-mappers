  ...
  for (var i = 0; i < l; i++) {
    ...
    if (c.nodeType === 3) {
      ...
      if (child.nodeName === 'if') {
        ...
        if (test && test.length > 0) {
          ...
          if (s2) {
            ...
            var separator = child.getAttribute('separator');
            var sub = { type: s2.type, text: v, property: s2.property, value: s2.value, format: format, array: array, separator: separator, prefix: prefix, suffix: suffix };
            templates.push(sub);
      ...
      if (isValidNode(child.nodeName)) {
        ...
        var sub = { property: property, encode: encode, value: value, text: subText, type: type, format: format, array: array, separator: separator, prefix: prefix, suffix: suffix };
        templates.push(sub);
        text = text + child.toString();
      }
    }
