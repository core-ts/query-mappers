  ...
  for (var i = 0; i < l; i++) {
    ...
    if (c.nodeType === 3) {
      ...
      if (child.nodeName === 'if') {
        ...
        if (test && test.length > 0) {
          ...
          if (s2) {
            ...
            var format = core_1.buildFormat(v);
            var sub = { type: s2.type, text: v, property: s2.property, value: s2.value, format: format };
            templates.push(sub);
      ...
      if (isValidNode(child.nodeName)) {
        ...
        var sub = { property: property, encode: encode, value: value, text: subText, type: type, format: format };
        templates.push(sub);
        text = text + child.toString();
      }
    }
